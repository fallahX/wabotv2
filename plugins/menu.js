let handler  = async (m, { conn, usedPrefix: _p }) => {
  let preview = {}
  try {
    if (!conn.menu) preview = await conn.generateLinkPreview('')
  } catch (e) {
    try {
      if (!conn.menu) preview = await global.conn.generateLinkPreview('')
    } catch (e) {}
  } finally {
...
+1 Exp/pesan biasa
+10 Exp/command
exp akan di reset ketika bot off
...
Owner Nomor:
${_p}bcgc <teks>
${_p}setmenu <teks> (Semua command %akan diubah menjadi prefix bot)
